<?php
class ServiceCordinator_DetailView_Model extends Vtiger_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
		...
		if (empty($acceptStatus)) {
			$basicActionLink = array(
				'linktype' => 'DETAILVIEWBASIC',
				'linklabel' => 'Approve',
				'linkurl' => "javascript:ServiceCordinator_Detail_Js.approveOrReject('index.php?module=" . $this->getModule()->getName() .
					"&action=ApproveOrReject&record=$recordId&source_module=ServiceCordinator','Accepted')",
				'linkicon' => ''
			);
			...
			$basicActionLink = array(
				...
				'linklabel' => 'Reject',
				'linkurl' => "javascript:ServiceCordinator_Detail_Js.approveOrReject('index.php?module=" . $this->getModule()->getName() .
					"&action=ApproveOrReject&record=$recordId&source_module=ServiceCordinator', 'Rejected')",
				'linkicon' => ''
			);
			$linkModelList['DETAILVIEWBASIC'][] = Vtiger_Link_Model::getInstanceFromValues($basicActionLink);
		}
		...
		if($acceptStatus == 'Accepted' && $currentUserModel->isAdminUser()){
			$userId = $this->getAssociatedUserId($recordModel->get('serv_code'));
			$serviceEngid = $recordModel->get('id');
			$passwordChangeUrl="ServiceCordinator_Detail_Js.triggerChangePassword('index.php?module=ServiceCordinator&action=SaveAjaxSavePassword&userid=$userId&serviceEngid=$serviceEngid')";
			$basicActionLink = array(
				'linktype' => 'DETAILVIEWBASIC',
