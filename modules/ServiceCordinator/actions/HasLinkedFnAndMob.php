...
class ServiceCordinator_HasLinkedFnAndMob_Action extends Vtiger_IndexAjax_View {
	...
	/*public function process(Vtiger_Request $request) {
		...
		if (($data && $data['cust_role'] == 'Service Manager' && $data['sub_service_manager_role'] == 'Regional Manager')) {
			$noFunctionalLocation = false;
		}
		if (($data && ($data['cust_role'] == 'BEML Management' || $data['cust_role'] == 'BEML Marketing HQ' ))) {
			$noFunctionalLocation = false;
		}
		if (
