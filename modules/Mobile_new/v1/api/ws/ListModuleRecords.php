...
class Mobile_WS_ListModuleRecords extends Mobile_WS_Controller {
	...
	function process(Mobile_API_Request $request) {
		...
		if (!empty($request->get('search_params'))) {
			...
			$i = 0;
			$modSearchParams = [];
			$keyOfAnotherMods = array_keys($referencendCols);
			foreach ($searchParams as $listSearchParam) {
				if (in_array($listSearchParam[0], $keyOfAnotherMods)) {
					$listSearchParam[0] = $referencendCols[$listSearchParam[0]];
				}
				if ($listSearchParam[0] == 'report_status') {
					if ($module == 'ServiceReports') {
						if ($listSearchParam[2] == 'Submitted') {
							$listSearchParam[2] = '1';
							$listSearchParam[0] = 'is_submitted';
							array_push($modSearchParams, ["is_recommisionreport", "e", "0"]);
						} else if ($listSearchParam[2] == 'In Progress') {
							$listSearchParam[2] = '0';
							$listSearchParam[0] = 'is_submitted';
							array_push($modSearchParams, ["is_recommisionreport", "e", "0"]);
						} else if ($listSearchParam[2] == 'Closed : Recommissioning Is Pending') {
							$listSearchParam[0] = 'is_submitted';
							$listSearchParam[2] = '1';
							array_push($modSearchParams, ["is_recommisionreport", "e", "1"]);
							array_push($modSearchParams, ["is_recc_submitted", "e", "0"]);
						} else if ($listSearchParam[2] == 'Closed') {
							$listSearchParam[0] = 'is_submitted';
							$listSearchParam[2] = '1';
							array_push($modSearchParams, ["is_recommisionreport", "e", "1"]);
							array_push($modSearchParams, ["is_recc_submitted", "e", "1"]);
						}
					} else if ($module == 'RecommissioningReports') {
						if ($listSearchParam[2] == 'Submitted') {
							$listSearchParam[2] = '1';
							$listSearchParam[0] = 'is_submitted';
						} else if ($listSearchParam[2] == 'In Progress') {
				...
				$i = $i + 1;
				array_push($modSearchParams,$listSearchParam);
			}
			$searchParams = array($modSearchParams);
				...
				foreach ($data as $i => $value) {
					if (is_string($i)) {
						...
						if (in_array($i, $referenceFields)) {
							...
							if ($record[$i] == '1') {
								if ($data['is_enginereport'] == '1') {
									$record['report_status'] = 'Submitted';
									$record['report_url'] = $site_URL_NonHttp . "modules/Mobile/v1/DownloadPDFReport?module=PDFMaker&source_module=ServiceReports&action=IndexAjax&record=$recordID&mode=getPreviewContent&language=en_us&generate_type=attachment&igtempid=7&access_token=" . $request->get('access_token') . "&useruniqueid=" . $request->get('useruniqueid');
								} else {
									$record['report_status'] = 'Submitted';
									$record['report_url'] = $site_URL_NonHttp . "modules/Mobile/v1/DownloadPDFReport?module=PDFMaker&source_module=ServiceReports&action=IndexAjax&record=$recordID&mode=getPreviewContent&language=en_us&generate_type=attachment&igtempid=12&access_token=" . $request->get('access_token') . "&useruniqueid=" . $request->get('useruniqueid');
								}
							} else {
								if ($data['is_enginereport'] == '1') {
							...
							if ($data['is_recc_submitted'] == '1') {
								$record['report_status'] = 'Closed';
							}
						} else if ($i == 'notesid') {
	...
	function getConfiguredStatusFields($statusFilterName, $moduleName) {
		...
		while ($row = $adb->fetch_array($result)) {
			...
			if(strpos($columnname[2], ';') !== false){
				$MetaMainColArr = explode(';', $columnname[2]);
				$MetaMainCol = $MetaMainColArr[0];
				$MetaMainCol = explode('(', $MetaMainCol);
				$c = explode(' ', $MetaMainColArr[1]);
				$f = explode(')', $c[2]);
				$d = $f[0];
				$referencendCols[trim($MetaMainCol[1]).trim($d)] = $columnname[2];
			}
