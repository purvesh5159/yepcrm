...
function createUserOnApproval($entityData) {
	...
	if ($rowCount > 0) {
		...
		if ($ajaxEditingInSEmod) {
			...
			$response->setError('Engineer is Already Exits, User Is Not Created');
			$response->emit();
			exit();
		} else {
			$viewer = new Vtiger_Viewer();
			$viewer->assign('MESSAGE', "Engineer is Alraedy Exits");
	...
	$focus = new Users();
	$password = $data['engineer_code'].'@123';
	$focus->column_fields['user_name'] =   $data['engineer_code'];
